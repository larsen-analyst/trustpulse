# TrustPulse — Project Handover Document v5
...
**Previous handover:** TrustPulse_Handover_v4.md (superseded by this document)
...
**Current state of repo (all committed and pushed as of this handover):**
- All pipeline ingest scripts complete including finance.py
- join.py updated with staff survey, vacancy benchmarks, finance data
- Full Flask app (app/, templates/, run.py) committed
- validate.py committed
- TrustPulse_Handover_v4.md committed
- TrustPulse_ProjectTracker_v8.html committed
- Latest commit: Phase 9c
...
│   │   ├── vacancies\
│   │   ├── staff_survey\
│   │   ├── finance\           (PDFs ingested via OCR)
│   │   ├── ambulance\         (FOLDER EXISTS — file not yet downloaded)
...
│       ├── finance_clean.csv          (NEW — 271 rows, 266 providers, 89 in deficit)
│       ├── trust_master.csv           (9,624 rows, 221 trusts, 107 cols)
│       ├── trust_profiles.csv         (221 rows, 256 cols)
...
│       ├── vacancies.py
│       ├── staff_survey.py
│       ├── finance.py                 (NEW this session)
│       ├── ambulance.py               (MISSING — next to write)
...
**venv location:** `D:\Projects\TrustPulse\venv\`
...
openpyxl, xlrd (2.0.2), odfpy (1.4.1), pytesseract, Pillow, pymupdf
```

**Tesseract OCR (Windows):**
- Installed at: `C:\Program Files\Tesseract-OCR\tesseract.exe`
- Required for finance.py
- finance.py sets: `pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"`

**IMPORTANT — PATH issue:**
There was a file called `python` in the project root that was interfering with the venv. It has been deleted. Always activate the venv before running any scripts.
...
### Phase 7d: Master join and analysis engine (UPDATED this session)

**trust_master.csv:** 9,624 rows, 221 trusts, 107 columns (was 81)
- Full date range April 2022 to March 2026
- All 13 datasets joined on trust code and date
- New this session: finance_clean.csv, staff_survey_clean.csv, vacancies_clean.csv all joined

**trust_profiles.csv:** 221 rows, 256 columns (was 187)
...
**Top 10 highest-risk trusts (March 2026) — UNCHANGED from v4:**
...
## New Datasets Added This Session (Phase 9)

### finance_clean.csv (NEW)
- 271 rows total, 266 provider rows
- 7 NHS England regions, 35 ICS
- Year: 2025/26 Q3
- 89 providers in deficit
- Key columns: trust_name, ics_name, region, ytd_plan_inc_dsf_m, ytd_actual_inc_dsf_m, ytd_var_m, var_pct_turnover, full_year_plan_exc_dsf_m, forecast_outturn_exc_dsf_m, forecasting_receipt_dsf, in_deficit, row_type
- Source PDF: Financial_performance_report_2025-26Q3.pdf (100-page raster PDF, no text layer)
- Extraction method: OCR via pytesseract + pymupdf, bounding-box column detection
- Trust names have OCR word-split artefacts — resolved by fuzzy matching in join.py
- Fuzzy match rate: 135/266 provider rows matched to org_code (difflib, cutoff 0.6)
- Script: pipeline/ingest/finance.py
- Runtime: approximately 5-8 minutes (OCR on 96 pages)

**finance.py technical notes:**
- Uses pymupdf (fitz) to rasterize pages — NOT pdftoppm (Linux only, not available on Windows)
- Requires Tesseract installed at C:\Program Files\Tesseract-OCR\tesseract.exe
- Column x-center boundaries calibrated at 150 DPI on 612pt letter-size page
- Pages 1-4 skipped (national summary tables, not trust rows)
- PDF was printed-to-PDF from browser — confirmed no text layer via pdffonts

### Staff survey columns added to trust_master.csv
- 13 metrics per trust, most recent year (2025) as snapshot
- Column names: survey_pp1_compassionate_inclusive through survey_theme_morale
- Coverage: 26% null (74% of trusts have survey data)
- Join key: org_code

### Vacancy benchmark columns added to trust_master.csv
- Regional benchmark only — no trust-level public data exists
- Columns: vac_benchmark_acute_all_pct, vac_benchmark_acute_nursing_pct
- Coverage: 33% null (matching trusts with fin_region)
- Region name mapping required (finance uses "North West Region", vacancies uses "North West")

### Finance columns added to trust_master.csv
- Prefixed fin_ — e.g. fin_forecast_outturn_exc_dsf_m, fin_in_deficit
- Coverage: 33-43% null depending on column

---

## Known Data Issues

1. sickness_rate_musculoskeletal and sickness_rate_infectious are 100% null — S11/S12/S13 reason codes not present in the sickness CSV. Not a blocker.
2. inpatients_60day_los_pct is 94% null — pre-existing issue, not new.
3. Finance fuzzy match rate is 135/266 — OCR word-splits reduce match quality. Trusts not matched will show NaN for all fin_ columns. Not a blocker.
4. ae_12hr columns are missing from A&E (39% null on type1 metrics) — pre-existing.

---

## Next Steps — Immediate Priority (New Chat)

### Task 4a — Ambulance handover delays (NEXT UP)
Download the ambulance handover timeseries file from:
```
...
Click: **"Web File Timeseries – Ambulance Collection"**

Save to: `data\raw\ambulance\`

Then upload the file to Claude and write `pipeline/ingest/ambulance.py`.

**What to extract:** Trust-level ambulance handover delay data
- Handovers over 15 minutes
- Handovers over 30 minutes
- Handovers over 60 minutes
- Total handovers
- Trust name and org code

**After ingesting:** Update join.py to include ambulance_clean.csv as a time series join on org_code + month. Then re-run join.py and analyse.py.

### Task 4b — Outpatient DNA rates
- Source: NHS Digital annual Excel
- Currently blocked — URL not confirmed
- Lower priority than ambulance

### Task 4c — GIRFT data
- Trust-level readmission and LOS outliers
- Not downloaded

### Task 4d — PHE Fingertips preventable admissions
- API or CSV available
- Not downloaded

### Task 5 — Deployment to Render (Phase 9 final)
Once data is complete:
1. Create requirements.txt
2. Add Gunicorn to requirements
3. Create render.yaml or configure via Render dashboard
4. Push to GitHub
5. Connect repo to Render
6. Set environment variables if needed
7. Verify app runs on public URL
...
| NHS Financial positions 2025/26 Q3 | Ingested | processed/finance_clean.csv |
| Ambulance handover delays | NOT DOWNLOADED | data/raw/ambulance/ folder exists |
...
17. Financial performance reports contain trust-level £m figures but as raster PDF (no text layer) — OCR required
18. pdftoppm is Linux only — use pymupdf (fitz) to rasterize pages on Windows
19. Tesseract must be installed separately on Windows — pytesseract is just the Python wrapper
20. Finance PDF column x-boundaries calibrated at 150 DPI: name <230, ytd_plan 240-320, ytd_actual 350-430, ytd_var 460-540, var_pct 570-650, allocation 690-770, full_year_plan 800-875, forecast_outturn 910-990, forecasting_dsf 1030-1120
21. Vacancy CSV uses quarter_date column (not date) and value column (not vacancy_rate_pct) with data_type distinguishing fte vs rate
22. Staff survey CSV column names are full snake_case (pp1_compassionate_inclusive etc.) not short codes (PP1 etc.)
23. Finance region names differ from vacancy region names — mapping table required in join.py
24. There was a file called python in project root interfering with PATH — deleted
...
12. Finance PDF tables have trust names but no org codes — fuzzy matching used in join.py.
...
15. Always activate venv before running scripts: venv\Scripts\activate
16. finance.py requires Tesseract installed on Windows and pytesseract.pytesseract.tesseract_cmd set.
17. finance.py uses pymupdf for rasterization, not pdftoppm.
...
| Ambulance handover delays | england.nhs.uk/statistics/statistical-work-areas/uec-sitrep/urgent-and-emergency-care-daily-situation-reports-2025-26/ |
